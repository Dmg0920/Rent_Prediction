# 進階統計分析與機器學習功能

本專案展示了**統計驅動的機器學習方法**，結合嚴謹的統計推論與現代機器學習技術。

## 📊 核心功能

### 1. 統計特徵分析 (`analysis/feature_analyzer.py`)

#### 1.1 相關性分析
- **Pearson 相關係數**：檢測線性關係
- **Spearman 秩相關**：檢測單調關係
- 自動標註顯著性水準（***強相關，**中等相關，*弱相關）

#### 1.2 多重共線性診斷 (VIF)
- **變異數膨脹因子 (Variance Inflation Factor)**
- VIF > 10 表示嚴重共線性
- 自動標註需要處理的特徵

#### 1.3 互信息分析
- 檢測**非線性相依關係**
- 比Pearson相關更全面
- 適合發現複雜的特徵-目標關係

#### 1.4 卡方獨立性檢驗
- 測試類別變數與目標的關聯性
- 提供p值和統計顯著性判斷

### 2. 集成學習模型 (`models/ensemble_models.py`)

#### 支援的模型：
1. **Random Forest**：基於決策樹的集成方法
2. **Gradient Boosting**：梯度提升樹
3. **XGBoost**：極限梯度提升（需安裝xgboost）
4. **LightGBM**：輕量級梯度提升（需安裝lightgbm）

#### 功能：
- **特徵重要性分析**：識別最重要的預測因子
- **交叉驗證**：更穩健的效能評估
- **模型集成**：結合多個模型的預測（平均或加權）
- **自動效能比較**：R², RMSE, MAE, MAPE

### 3. 統計假設檢驗 (`analysis/model_evaluator.py`)

#### 3.1 配對 t 檢驗
- 比較兩個模型的預測誤差是否有顯著差異
- 提供t統計量、p值和信賴區間

#### 3.2 Diebold-Mariano 檢驗
- 專門用於預測準確度比較
- 適合時間序列和一般迴歸模型

#### 3.3 Friedman 檢驗
- 非參數方法，比較3個以上模型
- 不需要常態性假設

#### 3.4 Bootstrap 信賴區間
- 估計模型效能指標的不確定性
- 提供95%信賴區間
- 支援任意評估指標

#### 3.5 學習曲線分析
- 診斷**偏差-變異數權衡**
- 識別過擬合或欠擬合問題
- 判斷是否需要更多數據

#### 3.6 殘差分析
- 殘差圖：檢查變異數齊一性
- Q-Q圖：檢驗常態性假設
- 直方圖：視覺化誤差分佈

## 🚀 使用方式

### 安裝依賴
```bash
pip install -r requirements.txt
```

### 執行進階分析
```bash
cd ml
python scripts/advanced_analysis.py
```

### 在自己的代碼中使用

#### 1. 特徵分析
```python
from analysis.feature_analyzer import FeatureAnalyzer

analyzer = FeatureAnalyzer(df, target_col='總額元')

# 相關性分析
corr = analyzer.correlation_analysis(method='pearson', top_n=20)

# VIF分析
vif = analyzer.vif_analysis(threshold=10)

# 互信息分析
mi = analyzer.mutual_information_analysis(top_n=20)
```

#### 2. 訓練集成模型
```python
from models.ensemble_models import EnsembleModelTrainer

trainer = EnsembleModelTrainer(random_state=42)

# 訓練所有模型
results = trainer.train_all_models(
    X_train, y_train, X_test, y_test, use_log=True
)

# 查看特徵重要性
importance = trainer.get_feature_importance(
    'Random Forest', feature_names, top_n=20
)

# 比較模型
comparison = trainer.compare_models()
```

#### 3. 統計檢驗
```python
from analysis.model_evaluator import ModelEvaluator

evaluator = ModelEvaluator(random_state=42)

# 配對t檢驗
t_result = evaluator.paired_t_test(pred1, pred2, y_test)

# Bootstrap信賴區間
ci = evaluator.bootstrap_confidence_interval(
    y_test, predictions,
    metric_func=lambda y, p: np.sqrt(mean_squared_error(y, p)),
    n_bootstrap=1000
)

# 學習曲線
lc = evaluator.learning_curve_analysis(model, X, y, cv=5)
```

## 📈 輸出示例

### 相關性分析
```
============================================================
相關性分析 (Pearson Correlation)
============================================================

與 總額元 相關性最高的前 15 個特徵：

 1. 坪數                          :  0.8234 ***
 2. 建物總面積平方公尺              :  0.7891 ***
 3. 建物現況格局-房                :  0.6543 ***
 4. 有電梯                        :  0.5234 ***
 5. 總樓層數                      :  0.4567 **
...
```

### VIF分析
```
============================================================
多重共線性診斷 (VIF Analysis)
============================================================

特徵                           VIF         診斷
------------------------------------------------------------
建物總面積平方公尺              45.67      ⚠️ 嚴重共線性
坪數                           42.34      ⚠️ 嚴重共線性
建物現況格局-房                 8.23      ⚡ 中等共線性
總樓層數                        3.45      ✓ 正常
...
```

### 模型比較
```
============================================================
模型效能比較
============================================================

模型                    R²         RMSE          MAE       MAPE
----------------------------------------------------------------------
XGBoost            0.8456      4,234         3,123     12.34%
Random Forest      0.8234      4,567         3,456     13.45%
Gradient Boosting  0.8123      4,789         3,678     14.56%
```

### 統計檢驗
```
============================================================
配對 t 檢驗 (Paired t-test)
============================================================

假設檢驗：
  H₀: 兩模型平均誤差相同
  H₁: 兩模型平均誤差不同
  顯著水準 α = 0.05

檢驗結果：
  t 統計量 = 3.4567
  p 值 = 0.0012
  平均誤差差異 = 234.56
  誤差差異標準差 = 456.78

結論: ✓ 拒絕 H₀ (p < 0.05)
  兩模型的預測誤差有顯著差異
```

## 🎯 專業價值

這個進階分析框架展示了以下專業技能：

### 統計學專業
1. **假設檢驗**：t檢驗、卡方檢驗、Friedman檢驗
2. **多變量分析**：相關性、共線性診斷
3. **非參數方法**：Spearman相關、秩檢驗
4. **重抽樣方法**：Bootstrap信賴區間
5. **模型診斷**：殘差分析、Q-Q圖

### 機器學習專業
1. **集成學習**：Random Forest、Boosting、Stacking
2. **特徵工程**：互信息、特徵重要性
3. **模型評估**：交叉驗證、學習曲線
4. **不確定性量化**：預測區間、信賴區間

### 實務應用能力
1. **模型比較**：嚴謹的統計檢驗，而非僅憑指標
2. **可解釋性**：特徵重要性、SHAP值基礎
3. **穩健性**：Bootstrap、交叉驗證
4. **診斷能力**：識別過擬合、欠擬合、共線性

## 📚 參考文獻

- Diebold, F. X., & Mariano, R. S. (1995). Comparing predictive accuracy.
- Friedman, M. (1937). The use of ranks to avoid the assumption of normality.
- Efron, B., & Tibshirani, R. J. (1994). An introduction to the bootstrap.
- Chen, T., & Guestrin, C. (2016). XGBoost: A scalable tree boosting system.

## 🔧 進一步擴展

可以考慮添加：
1. **SHAP值分析**：更進階的模型解釋
2. **分位數迴歸**：預測租金區間
3. **貝葉斯方法**：參數後驗分佈
4. **因果推論**：傾向分數匹配
5. **自動特徵選擇**：遞迴特徵消除（RFE）
